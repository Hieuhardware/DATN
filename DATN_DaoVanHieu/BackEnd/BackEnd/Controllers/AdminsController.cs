...
namespace BackEnd.Controllers
{
    ...
    public class AdminsController : ControllerBase
    {
		...
		[HttpGet("datachartpie/month")]
		public IActionResult GetDataOfProductByMonth(int month, int year)
		{
			try
			{
				using (conn = new MySqlConnection(SqlConnectionString.ConnectionString))
				{
                        ...
                        string orderString2 = "SELECT prod_id, name, SUM(quantity) AS total_quantity_sold FROM orderdetailview WHERE MONTH(order_date) = @month AND YEAR(order_date) = @year  AND status = 'Đã giao hàng' GROUP BY prod_id, name ORDER BY total_quantity_sold DESC LIMIT 5";
						var result= conn.Query(orderString2, param: new { month = month, year = year });
                    return Ok(
                        result);
				}
			}
		...
		[HttpGet("datachartpie/year")]
		public IActionResult GetDataOfProductByYear(int year)
		{
			try
			{
				using (conn = new MySqlConnection(SqlConnectionString.ConnectionString))
				{
					...
					string orderString2 = "SELECT prod_id, name, SUM(quantity) AS total_quantity_sold FROM orderdetailview WHERE YEAR(order_date) = @year  AND status = 'Đã giao hàng' GROUP BY prod_id, name ORDER BY total_quantity_sold DESC LIMIT 5";
					var result = conn.Query(orderString2, param: new { year = year });
					return Ok(
						result);
				}
			}
