    ...
    public class CartsController : ControllerBase
    {
        ...
        public IActionResult Put(Cart c)
        {
            try
            {
                using (conn = new MySqlConnection(SqlConnectionString.ConnectionString))
                {
					var getProductAmountSql = "SELECT amount FROM Product WHERE id = @prod_id";
					var productAmount = conn.ExecuteScalar<int>(getProductAmountSql, new { prod_id = c.prod_id });
					if (c.quantity <= productAmount)
                    {
                    ...
                    }else
                    {
						...
						{
							message = "Số lượng sản phẩm trong giỏ hàng vượt quá số lượng sản phẩm hiện có."
						});
					}
