<template>
  <div>
    <div class="view">
      <div class="view-title">Quản lý đơn hàng</div>
      <div class="card">
        <div class="card-item">
          ...
          <div class="card-item-content">
            ...
            <button class="button button-secondary" @click="refresh">
              Làm mới
            </button>
          </div>
        </div>
    ...
    <div class="nodata" v-if="noData">Không có dữ liệu để hiển thị!</div>
...
export default {
  ...
  data() {
    return {
      tableHeader: [
        ...
        { id: "order_date", name: "Ngày đặt hàng" },
        { id: "status", name: "Trạng thái" },
        { id: "tel", name: "Số điện thoại" },
      ],
  ...
  methods: {
    ...
    deleteItem(item) {
      this.item = item;
      this.title = `Bạn có thật sự muốn xóa mục này không.`;
      this.hasQuestion = true;
    },
    yes() {
      this.$axios
        .delete(`${api.OrderApi}/${this.item.id}`)
        .then(() => {
          this.title = "Cập nhật hệ thống thành công.";
          this.hasToast = true;
          setTimeout(() => {
    ...
    getOrders() {
      ...
      this.hasLoader = true;
      this.$axios
        .get(`${api.OrderApi}/pagination`, { params: param })
        .then((res) => {
